#include "conv2pool2.h"
void LetNet2(volatile float *addrMaster,int* addrSlave);
		...
		for(i_y = 0; i_y < 10; i_y++){
			for(i_x = 0; i_x < 10; i_x++){
				...
				for(k_num = 0; k_num < 6; k_num++){
					float matrix_2[25];
					for(mat_i = 0;mat_i<25;mat_i++){
						// int weights_index = mat_i + k_num*25 + (nk_num+1)*150; //参数从第二层卷积�?始不�?�?+1
...
void LetNet2(volatile float *addrMaster,volatile float* addrSlave){
#pragma HLS INTERFACE m_axi depth=3576 port=addrMaster offset=slave bundle=input
#pragma HLS INTERFACE m_axi depth=400 port=addrSlave offset=slave bundle=output
...
#pragma HLS INTERFACE s_axilite port=return bundle=CRTL_BUS
	...
	float data[3576]; //2400卷积�?+1176 A2_value
	...
	memcpy(data,(const float*)addrMaster,3576*sizeof(float));
	//get the image data
	for(loop1_i = 0; loop1_i<1176; loop1_i++){
		//photo[loop1_i] = data[loop1_i+2400];
		A2_value[loop1_i] = data[loop1_i+2400];
	}
...
/* 	ConvLayer_1(photo,C1_value,data);                  //122,304   FLOPs
	...
	memcpy((float*)addrSlave,(const float*)A4_value,400*sizeof(float));
