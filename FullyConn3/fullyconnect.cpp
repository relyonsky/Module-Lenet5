...
void LetNet3(volatile float *addrMaster,int* addrSlave);
...
void FullyConnLayer_6(float input[120],float *F6_value,float * weights){
	...
	for(i_y = 0; i_y < 84; i_y++){
		...
		for(i_x = 0;  i_x < 120; i_x++){
			...
			res += input[i_x] * weights[index + 48000];
		}
		F6_value[i_y] = res;
	}
}

//kernel 10x120 = 1200 59280
void FullyConnLayer_7(float input[84],float *F6_value,float * weights){
	...
	for(i_y = 0; i_y < 10; i_y++){
		...
		for(i_x = 0;  i_x < 84; i_x++){
			...
			res += input[i_x] * weights[index + 58080];
		}
		F6_value[i_y] = res;
	}
}
...
void LetNet3(volatile float *addrMaster,int* r){
#pragma HLS INTERFACE m_axi depth=59680 port=addrMaster offset=slave bundle=MASTER_BUS
...
#pragma HLS INTERFACE s_axilite port=return bundle=CRTL_BUS
	...
	memcpy(data,(const float*)addrMaster,59680*sizeof(float));
	//get the image data
	for(loop1_i = 0; loop1_i<400; loop1_i++){
		A4_value[loop1_i] = data[loop1_i+59280];
	}
